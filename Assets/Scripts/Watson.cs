...
{
    private string _username = "465e15df-1ec5-47e1-9e45-b0f79d8a8339";
    private string _password = "LR6xMoXBNcJo";
        ...
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                foreach (var alt in res.alternatives)
                {
                    ...
                    // string text = string.Format("{0} ({1}, {2:0.00})\n", alt.transcript, res.final ? "Final" + alt.confidence: "Interim", "");
                    // string text = string.Format("{0} ({1}, {2:0.00})\n", alt.transcript, res.final ? "Final" + alt.confidence: "Interim", "");
                    // Log.Debug("ExampleStreamingSplitSamples.OnRecognize()", text);
                    // print(text);
                    word = alt.transcript;
                    word = word.Trim();
                    pontos = alt.confidence;
                    // result = ResultsField;
                    // ResultsField.text = word + "(" + pontos + ")";
